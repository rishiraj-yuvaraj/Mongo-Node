...
app.get("/movies", async function(request, response){
    ...
    const movies = await client.db("Mongo-node").collection("movies").find(request.query).toArray();
    // console.log(movies);
    response.send(movies);
});
...
//     // const movie = await client.db("Mongo-node").collection("movies").findOne({ id : "102" });
   const movie = await client.db("Mongo-node").collection("movies").findOne({ id : id }); //getting this last id from line no 132 local params id
...
app.post("/movies", async function(request, response){
    ...
    console.log(data);
    const result = await client.db("Mongo-node").collection("movies").insertMany(data);
...
app.delete("/movies/:id", async function(request, response){
    ...
    const result = await client.db("Mongo-node").collection("movies").deleteOne({id : id});
    console.log(result);
...
app.put("/movies/:id", async function(request, response){
    ...
    const result = await client.db("Mongo-node").collection("movies").updateOne({id : id}, {$set : data });
    console.log(result);
    response.send(result);
